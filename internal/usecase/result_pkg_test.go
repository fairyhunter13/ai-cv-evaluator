...
func Test_makeETag_ChangesWithContent(t *testing.T) {
	...
	if etag1 == etag2 || etag1 == "" || etag2 == "" {
		t.Fatalf("etag not varying: %s %s", etag1, etag2)
	}
}
...
func Test_errWrapped(t *testing.T) {
	...
	if !errWrapped(wrapped, base) {
		t.Fatalf("expected wrapped to match base")
	}
}
...
func Test_errorCodeFromJobError(t *testing.T) {
	...
	for in, want := range cases {
		if got := errorCodeFromJobError(in); got != want {
			t.Fatalf("%q => %q (got %q)", in, want, got)
		}
	}
}
