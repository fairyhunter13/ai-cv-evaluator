...
func TestService_FilterFreeModels_ViaAPI(t *testing.T) {
	t.Parallel()

	// Create a test server that returns mixed free and paid models
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/models" {
			w.WriteHeader(http.StatusNotFound)
			return
		...
		resp := map[string]any{
			"data": []map[string]any{
				{"id": "free-model-1", "name": "Free Model 1", "pricing": map[string]string{"prompt": "0", "completion": "0", "request": "0", "image": "0"}},
				{"id": "paid-model-1", "name": "Paid Model 1", "pricing": map[string]string{"prompt": "0.001", "completion": "0.002", "request": "0", "image": "0"}},
				{"id": "free-model-2", "name": "Free Model 2", "pricing": map[string]string{"prompt": "", "completion": "", "request": "", "image": ""}},
				{"id": "openrouter/auto", "name": "OpenRouter Auto", "pricing": map[string]string{"prompt": "0", "completion": "0", "request": "0", "image": "0"}},
			},
		}
	...
	models, err := service.GetFreeModels(ctx)
	assert.NoError(t, err)
	// Should filter out paid models and banned models (openrouter/auto)
	// Only free-model-1 and free-model-2 should remain
	assert.Len(t, models, 2)

	// Verify only free models are returned
	for _, m := range models {
		assert.Contains(t, m.ID, "free-model")
	}
}

func TestService_FilterFreeModels_AllPaidViaAPI(t *testing.T) {
	t.Parallel()

	// Create a test server that returns only paid models
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/models" {
			w.WriteHeader(http.StatusNotFound)
			return
		...
		resp := map[string]any{
			"data": []map[string]any{
				{"id": "paid-1", "name": "Paid 1", "pricing": map[string]string{"prompt": "0.001", "completion": "0.001"}},
				{"id": "paid-2", "name": "Paid 2", "pricing": map[string]string{"prompt": "0.002", "completion": "0.002"}},
			},
		}
		_ = json.NewEncoder(w).Encode(resp)
	}))
...
func TestService_GetFreeModels_CachedModels(t *testing.T) {
	t.Parallel()

	service := NewService("test-key", "http://unused", 1*time.Hour)
	ctx := context.Background()

	// Pre-populate with cached models (already filtered)
	service.models = []Model{
		{ID: "cached-free-1", Name: "Cached Free 1", Pricing: Pricing{Prompt: "0", Completion: "0"}},
		{ID: "cached-free-2", Name: "Cached Free 2", Pricing: Pricing{Prompt: "0", Completion: "0"}},
	}
	service.lastFetch = time.Now()
	...
	assert.NoError(t, err)
	assert.Len(t, models, 2)
	assert.Equal(t, "cached-free-1", models[0].ID)
}
