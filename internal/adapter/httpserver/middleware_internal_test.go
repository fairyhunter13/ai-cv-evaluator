...
func Test_allowedExt(t *testing.T) {
	t.Run("accepts", func(t *testing.T) {
		for _, n := range []string{"cv.txt", "doc.PDF", "report.Docx"} {
			if !allowedExt(n) { t.Fatalf("should allow %s", n) }
		}
	})
	t.Run("rejects", func(t *testing.T) {
		for _, n := range []string{"evil.exe", "img.png", "cv"} {
			if allowedExt(n) { t.Fatalf("should reject %s", n) }
		}
	})
}

func Test_allowedMIME(t *testing.T) {
	if !allowedMIME("text/plain") { t.Fatalf("expected to allow text/plain") }
	if !allowedMIME("text/plain; charset=utf-8") { t.Fatalf("expected to allow text/plain charset") }
	if !allowedMIME("application/pdf") { t.Fatalf("expected to allow pdf") }
	if !allowedMIME("application/vnd.openxmlformats-officedocument.wordprocessingml.document") { t.Fatalf("expected to allow docx") }
	if allowedMIME("application/octet-stream") { t.Fatalf("should not allow octet-stream") }
}
...
func Test_BuildResultEnvelope(t *testing.T) {
	...
	if m["id"].(string) != "id1" { t.Fatalf("id mismatch") }
	if m["status"].(string) != string(domain.JobCompleted) { t.Fatalf("status mismatch") }
	inner := m["result"].(map[string]any)
	if inner["project_score"].(float64) != 9 { t.Fatalf("score mismatch") }

	m2 := BuildResultEnvelope("id2", domain.JobQueued, nil)
	if _, ok := m2["result"]; ok { t.Fatalf("queued should not include result") }
}
