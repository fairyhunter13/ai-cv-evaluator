...
func Test_HashPassword_VerifyPassword(t *testing.T) {
	...
	if !VerifyPassword("s3cret", hash) { t.Fatalf("verify failed") }
	if VerifyPassword("wrong", hash) { t.Fatalf("verify should fail for wrong password") }
}
...
func Test_SessionManager_Create_Validate_And_Middleware(t *testing.T) {
	...
	if err != nil || sd == nil || sd.Username != "admin" { t.Fatalf("validate failed: %v %+v", err, sd) }
	...
	if rw.Result().StatusCode != http.StatusSeeOther || nextCalled { t.Fatalf("expected redirect for missing cookie") }
	...
	if !nextCalled { t.Fatalf("expected next to be called with valid session") }
}

func Test_parseInt64(t *testing.T) {
	if parseInt64("123") != 123 { t.Fatalf("parse 123") }
	if parseInt64("x") != 0 { t.Fatalf("parse invalid should be 0") }
}
