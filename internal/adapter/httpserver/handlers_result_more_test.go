...
func TestResultHandler_406_NotAcceptable(t *testing.T) {
	...
	s := httpserver.NewServer(cfg, upSvc, evSvc, resSvc, nil, nil, nil, nil)
	rw := httptest.NewRecorder()
	r := httptest.NewRequest("GET", "/v1/result/job1", nil)
	r.Header.Set("Accept", "text/plain")
	h := s.ResultHandler()
	h(rw, r)
	if rw.Result().StatusCode != http.StatusNotAcceptable {
		t.Fatalf("want 406, got %d", rw.Result().StatusCode)
	}
}
